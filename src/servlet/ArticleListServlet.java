 ...
 */
@WebServlet(description = "ArticleListServlet", urlPatterns = { "/ArticleListServlet" })
public class ArticleListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String sql1 = "select count(ID_) from article"; // 执行查询操作
			...
			// 查询出需要显示的记录
			String sql2 = "select top " + pageSize + " * from article where ID_ not in(select top "
					+ (pageSize * (pageNow - 1)) + " ID_ from article order by id_ desc)order by id_ desc"; // 执行查询操作
			...
			while (rs2.next()) { // 循环显示数据表中的数据
				...
				blog.setType_id(rs2.getInt("TYPE_ID"));
			...
			RequestDispatcher d = request.getRequestDispatcher("articlelist.jsp");

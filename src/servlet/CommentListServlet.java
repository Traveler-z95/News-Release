 ...
 */
@WebServlet(description = "CommentListServlet", urlPatterns = { "/CommentListServlet" })
public class CommentListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String sql1 = "select count(ID_) from comment"; // 执行查询操作
			...
			// 查询出需要显示的记录
			String sql2 = "select top " + pageSize + " * from comment where ID_ not in(select top "
					+ (pageSize * (pageNow - 1)) + " ID_ from comment order by id_ desc)order by id_ desc"; // 执行查询操作
			...
			RequestDispatcher d = request.getRequestDispatcher("commentlist.jsp");

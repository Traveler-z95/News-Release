...
/**
 * Servlet implementation class AfficheJsonServlet
 */
@WebServlet(description = "AfficheJsonServlet", urlPatterns = { "/AfficheJsonServlet" })
public class AfficheJsonServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public AfficheJsonServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String sql = "select * from affiche "; // 查询操作
			...
			for (int i = 0; i < list2.size(); i++) { // 将数据转换成json格式
				...
				jsonStringer.key("title").value(blog.getTitle());

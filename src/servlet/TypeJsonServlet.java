...
/**
 * Servlet implementation class TypeJsonServlet
 */
@WebServlet(description = "TypeJsonServlet", urlPatterns = { "/TypeJsonServlet" })
public class TypeJsonServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public TypeJsonServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String sql = "select * from type "; // 查询操作
			...
			for (int i = 0; i < list2.size(); i++) { // 将数据转换成json格式
				...
				jsonStringer.key("name").value(blog.getName());
				jsonStringer.key("description").value(blog.getDescription());

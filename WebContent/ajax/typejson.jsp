...
<script type="text/javascript">
	...
	function onResponse() {
		...
		//创建表格
		strText = "<table border='1'><tr><th>Id</th><th>Name</th><th>Description</th></tr>";
		for (var i = 0; i < jsonResult.length; i++) {//循环显示
			strText = strText + "<tr>";
			strText += '<td>' + jsonResult[i].id + '</td>' + " " + '<td>'
					+ jsonResult[i].name + '</td>' + " " + '<td>'
					+ jsonResult[i].description + '</td>';
...
<body>
	<div id=copy>
		<button onclick="loadXMLDoc('../TypeJsonServlet')">获取json</button>
	</div>
		<a href="../table/typetable.jsp">表格</a>
</body>
</html>
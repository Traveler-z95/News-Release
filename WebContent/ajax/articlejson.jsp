...
<script type="text/javascript">
	...
	function onResponse() {
		...
		//创建表格
		strText = "<table border='1'><tr><th>Id</th><th>type_id</th><th>Title</th><th>Content</th><th>Time</th></tr>";
		for (var i = 0; i < jsonResult.length; i++) {//循环显示
			strText = strText + "<tr>";
			strText += '<td>' + jsonResult[i].id + '</td>' + " " + '<td>'
					+ jsonResult[i].type_id + '</td>' + " " + '<td>'
					+ jsonResult[i].title + '</td>' + " " + '<td>'
...
<body>
	<div id="copy">
		<button onclick="loadXMLDoc('../ArticleJsonServlet')">获取json</button>
	</div>
	<a href="../table/articletable.jsp">表格</a>
</body>
</html>